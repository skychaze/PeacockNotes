...
export function runAttachmentReferenceTests() {
  ...
  assert(attachmentTokenPattern.test(token), 'stored tokens match the mentions pattern');
  assert(!attachmentTokenPattern.test('@[x](file:not-a-uuid)'), 'only UUIDv4 tokens count');
  ...
  assert(
    attachmentTextForSharing(content) === `Before @${name}\nafter @ordinary person@example.com`,
    'sharing text drops internal identity',
  );
  assert(
    attachmentTextForSharing('ordinary @text\nand email@host.com') === 'ordinary @text\nand email@host.com',
    ...
    audioGroups: [
      {
        ...
        segments: [
          { portableId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa' },
          { portableId: 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb' },
        ],
      },
    ],
    files,
  ...
  assert(
    tags[0].kind === 'audio' && tags[0].id === 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa' && tags[0].segmentCount === 2,
    'audio tags reference the first segment of their group',
  );
  ...
  assert(listAttachmentTags({ audioGroups: [{ groupId: 'g', displayName: 'x', segments: [] }], files: [] }).length === 0, 'a group without identity is not taggable');
  ...
  const renamed = rewriteAttachmentReferences(
    `Audio ${encodeAttachmentReference({ kind: 'audio', id, name: 'Old name' })} and file ${token} remain.`,
    (reference) => (reference.kind === 'audio' && reference.id === id
      ? encodeAttachmentReference({ ...reference, name: 'New name' })
      : undefined),
  );
  assert(
    renamed.includes(`@[New%20name](audio:${id})`) && !renamed.includes('(audio:' + id + ')@'),
    'matched tokens keep identity and take the new name',
  );
  assert(renamed.includes(token), 'declined tokens stay byte-for-byte identical');

  const unlinked = rewriteAttachmentReferences(`Keep ${token} but drop ${token.replace(id, otherId)}`, (reference) =>
    reference.id === id ? reference.name : undefined,
  );
  assert(
    unlinked === `Keep ${name} but drop ${token.replace(id, otherId)}`,
    'unlinking replaces only the matching token with its stored name',
  );
  assert(
    rewriteAttachmentReferences('no tokens here', () => 'unused') === 'no tokens here',
    'text without tokens is untouched',
  );
}

void Promise.resolve(runAttachmentReferenceTests()).catch((error: unknown) => {
  console.error(error);
  process.exitCode = 1;
});
