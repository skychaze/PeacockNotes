...
- an Expo module supplies the Android activity result for `ACTION_CREATE_DOCUMENT`, receives `content://` URIs, and does all archive, digest, and extraction I/O on a native worker;
...
Use `net.lingala.zip4j:zip4j:2.11.6`, pinned in the module Gradle file. Zip4j's own Maven metadata names 2.11.6 as its latest release, and its POM targets Java 8. [Maven metadata](https://repo1.maven.org/maven2/net/lingala/zip4j/zip4j/maven-metadata.xml) [POM](https://repo1.maven.org/maven2/net/lingala/zip4j/zip4j/2.11.6/zip4j-2.11.6.pom) Its stream classes take `InputStream` and `OutputStream`, and its header writer emits ZIP64 records when a size or entry count crosses the ZIP32 limits. [Zip output stream](https://github.com/srikanth-lingala/zip4j/blob/master/src/main/java/net/lingala/zip4j/io/outputstream/ZipOutputStream.java) [ZIP64 header writer](https://github.com/srikanth-lingala/zip4j/blob/master/src/main/java/net/lingala/zip4j/headers/HeaderWriter.java)

There is no single maintained Expo package that also provides ZIP64 streaming, SHA-256, hostile-archive policy, SAF document creation, and post-write document readback. The module owns those policy and SAF parts. Zip4j only owns ZIP parsing and encoding.
...
The legacy `StorageAccessFramework` namespace can select a tree and create a document in that tree, but it is not a save-as document flow and it still exposes JS-facing read and write calls rather than the required native archive pipeline. [Legacy SAF API](https://github.com/expo/expo/blob/599ebc94db5c972f961641db9aa33e2964498c42/packages/expo-file-system/src/legacy/FileSystem.ts)
...
For export, the module must launch `Intent.ACTION_CREATE_DOCUMENT` with `application/zip` and an `EXTRA_TITLE` ending in `.peacocknotes`. Android defines that action as allowing the user to create a document. This works through the selected DocumentsProvider, including Google Drive, without Google Drive API scopes. The app only receives a user-granted document URI. [Intent reference](https://developer.android.com/reference/android/content/Intent#ACTION_CREATE_DOCUMENT) [SAF guide](https://developer.android.com/training/data-storage/shared/documents-files)
...
1. The Kotlin module launches `ACTION_CREATE_DOCUMENT` and receives the destination URI.
2. Open `ContentResolver.openOutputStream(uri)` and wrap it in `DigestOutputStream(MessageDigest.getInstance("SHA-256"))`, then `ZipOutputStream`. Android explicitly provides `openOutputStream` and `openInputStream` for content URIs. [ContentResolver reference](https://developer.android.com/reference/android/content/ContentResolver)
3. For the sanitized SQLite file and every blob, read a fixed native buffer, update that entry's `MessageDigest`, and write the same buffer to Zip4j. Set the entry name exactly: `database.sqlite` and `blobs/sha256/<lowercase-hex-digest>`. Do not add directory entries, symlinks, arbitrary names, encryption, or signatures. Close each entry before storing its byte count and SHA-256 in an in-memory manifest record.
...
Before staging, reserve space for the configured maximum archive plus extraction or reject early. After header validation, reserve the declared extraction total plus a safety margin and check before every output file. Use `StatFs.getAvailableBytes()` on the app-private volume, not a provider-reported `COLUMN_SIZE`, which may be absent or stale. [StatFs reference](https://developer.android.com/reference/android/os/StatFs)

### Cancellation and cleanup

Expose an operation ID and `cancel(operationId)`. The operation owns its `CancellationSignal`, active `ParcelFileDescriptor`, streams, temporary archive, and staging directory. `cancel` signals cancellation and closes the active descriptor or stream to unblock a provider read or write; every buffer iteration also calls `throwIfCanceled()`. Android's `CancellationSignal` provides `cancel`, `isCanceled`, and `throwIfCanceled`. [CancellationSignal reference](https://developer.android.com/reference/android/os/CancellationSignal)
...
2. Exercise a Drive or other remote DocumentsProvider destination: create, stream write, close, native readback SHA-256, and compare.
