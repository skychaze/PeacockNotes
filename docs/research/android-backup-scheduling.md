...
This research covers Android only. It distinguishes Expo BackgroundTask from the deprecated Expo BackgroundFetch package and from WorkManager's public periodic-work model. Primary sources are the Expo SDK 54 documentation and source, current Android documentation, and this repository. Every material claim was re-verified against these current primary sources on 2026-09-10; that review added the restricted-bucket and Android 16 quota facts below.
...
A user force-stop is stronger. Android describes the stopped package state as persisting until direct or indirect user action launches or interacts with the app. Android 15 tightened this behavior and cancels pending intents while stopped; it delivers `BOOT_COMPLETED` only after user action removes the stopped state. See Android's [package stopped-state documentation](https://developer.android.com/about/versions/15/behavior-changes-all#enhanced-stop-states). [`ApplicationStartInfo.wasForceStopped()`](https://developer.android.com/reference/android/app/ApplicationStartInfo#wasForceStopped) lets the app detect a prior force-stop on its next launch, which is useful for explaining an overdue backup in the UI. No backup scheduler should claim to bypass force-stop. The app-open overdue check is the recovery path after the user starts Peacock Notes again.
...
App Standby and standby buckets further limit jobs for infrequently used apps. Android's [standby-bucket guide](https://developer.android.com/topic/performance/appstandby) says bucket assignment affects job frequency while on battery. On Android 13 and later, the system places an app the user has not interacted with for 8 days (45 days on Android 12) into the restricted bucket unless the app qualifies for an exemption. A restricted-bucket app may run jobs only once per day in a 10-minute batched session grouped with other apps' jobs, restricted jobs do not run by themselves, and the current [power limits table](https://developer.android.com/topic/performance/power/power-details#app-stdby-bucket) lists network access as disabled for the restricted bucket. The quota values are approximate rather than guaranteed, and Android 16 further changed regular and expedited job runtime quota based on the standby bucket and process state.
...
| App Standby or rare use | Android still allocates limited job opportunities according to standby bucket and device state. | Daily timing. Quota tables are approximate and policy may change. After 8 days without user interaction, Android 13+ may place the app in the restricted bucket with network access disabled. | Use the app-open overdue path and communicate staleness from the last success time. |
...
- [Android 15 package stopped-state behavior](https://developer.android.com/about/versions/15/behavior-changes-all#enhanced-stop-states)
- [`ApplicationStartInfo` reference](https://developer.android.com/reference/android/app/ApplicationStartInfo)
